...
class _NavigationScreenState extends State<NavigationScreen> {
  ...
  LatLng curLocation = LatLng(23.8103, 90.4125);
  ...
  void getNavigation() async {
    ...
    if(!_serviceEnabled) {
      ...
      if(!_serviceEnabled) {
        if (kDebugMode) print('<>><><><> Location service cannot be enabled <>><><><>');
        return;
      }
    }
      ...
      locationSubscription = location.onLocationChanged.listen((LocationData currentLocation) {
        controller?.animateCamera(
          ...
          CameraUpdate.newLatLngBounds(
            boundsFromLatLngList([curLocation, LatLng(widget.lat, widget.lng)]),
            // LatLngBounds(southwest: curLocation, northeast: LatLng(widget.lat, widget.lng)),
            50
          )
          ...
          setState(() {
            ...
            sourcePosition = Marker(
              ...
              infoWindow: InfoWindow(
                ...
                snippet: 'lat1 ${widget.lat} lng1 ${widget.lng} lat2 ${curLocation.latitude} lng2 ${curLocation.longitude}',
              ),
              onTap: () {
                if (kDebugMode) print('<>><><><> Location marker tapped <>><><><>');
              },
            );
          });
  ...
  double calculateDistance(lat1, lng1, lat2, lng2) {
    ...
    var a = 0.5 -
            c((lat2 - lat1) * p) /2 +
            c(lat1 * p) * c(lat2 * p) * (1 - c(lng2 - lng1)) / 2;
  ...
  Future<void> getDirections(LatLng dst) async {
    ...
    //   'AIzaSyD2C6aTveM49ngdulUtz_AhakrEKBk61tA',
    ...
    //   });
    // } else {
    //   if (kDebugMode) print(result.errorMessage);
    // }
    polylineCoordinates.add(curLocation);
    polylineCoordinates.add(dst);
